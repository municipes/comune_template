...
################################################################################
#
#  Questo è uno script che installa Comune Silfi CMS
#
#  Author: Pietro Arturo Panetta
#  Site: https://www.drupal.org/u/arturopanetta
#  Modified by: Maurizio Cavalletti
#  Site: https://www.drupal.org/u/maurizio_akabit
...
# Nome del progetto
read -r -p "Nome del progetto (fomato FQDN host) [a-z0-9] (mio-comune): " project_name
project_name=${project_name:-mio-comune}

while ! validate_project_name "$project_name"; do
  read -r -p "Nome del progetto (formato FQDN host) [a-z0-9] (mio-comune): " project_name
  project_name=${project_name:-mio-comune}
done
...
echo "------------------------------------"
ddev composer create municipes/comune_template:${municipesCodebaseInstallVersion} --no-install --no-cache
...
echo "----------------------------------------------"
ddev exec /var/www/html/scripts/setup_step01__municipes_codebase.sh
...
# echo " "
# n "Setup iniziale di Municipes" notice
# echo "------------------------------------"
# ddev exec /var/www/html/scripts/setup_step03__configure_municipes.sh
